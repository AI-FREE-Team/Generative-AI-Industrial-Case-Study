...
## 4. 平面設計領域使用情境
在創作時會遵循以下流程：靈感發想 > 草稿 > 線稿 > 上色 > 細調 > 提高作品被仿難度。而AI繪圖工具可以在每個階段都發揮不同的作用。
...
### 4.2 草稿 > 線稿 階段
在已透過PS完成部分草稿的情境中，像是「已完成畫面主體-前景人像，但背景尚未確定」的情境，可以透過 AI 繪圖工具嘗試同一種畫面主體，但不同背景樣式，如街道、櫻花盛開的公園、圖書館等。這部分同樣能以線稿形式，或是以上色後的形式呈現。決定背景後，也能夠回到PS中，用AI繪圖工具產出的結果進行調整，不一定要從0-1全靠自己。
...
### 4.4 細調
原則上，無論輸入內容是什麼（有時候甚至是只有文字），AI繪圖工具都能夠產出堪用甚至是令人驚豔的視覺意象，這時候都能夠回到PS中進行微調。

### 4.5 提高作品被仿難度
這部分可以透過PS加入一些肉眼看不太出來色板錯位+特效雜訊，或是讀者自己的品牌LOGO，讓產出的作品無法輕易的被盜用。